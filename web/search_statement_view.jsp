    ...
    <c:choose>
        <c:when test="${user.userRole == 'CLIENT'}">
            <c:if test="${empty search_statement}">
                <form action="addSearchStatement.action" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
            </c:if>
        </c:when>

        <c:otherwise>
            <c:if test="${not empty search_statement}">
                <form action="updateSearchStatement.action" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
