    ...
    <c:choose>
        <c:when test="${(user.userRole == 'ADMIN') || (user.userRole == 'POST_MANAGER')}">
            <c:if test="${not empty prepayment_book}">
                <form action="updatePrepaymentBook.action" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
            </c:if>
        </c:when>
        <c:otherwise>
            <c:if test="${empty prepayment_book}">
                <form action="client/addPrepaymentBook.action" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
            </c:if>
        </c:otherwise>
    </c:choose>
        ...
        <div class="row">
            <div class="input-field col s6">
                <c:if test="${empty prepayment_book}">
                    <input type="text" required name="client_name" value="${user.firstName} ${user.secondName}" placeholder="Полищук Д.А." maxlength="45" id="client_name" class="validate">
                </c:if>
                <c:if test="${not empty prepayment_book}">
